...
import time import logging import asyncio from typing import Optional import ccxt.async_support as ccxt

from config.settings import API_KEY, API_SECRET, USE_TESTNET, DRY_RUN

logger = logging.getLogger(name)

class BinanceClient: def init(self, api_key: str = API_KEY, api_secret: str = API_SECRET, use_testnet: bool = USE_TESTNET, dry_run: bool = DRY_RUN): self.dry_run = dry_run opts = {"defaultType": "future"}  # fuerza FAPI self.exchange = ccxt.binance({ "apiKey": api_key, "secret": api_secret, "enableRateLimit": True, "options": opts, })

if use_testnet:
        # Conectar al TESTNET de Binance Futures (USDT-M)
        # Las rutas deben incluir solo la base /fapi (ccxt agrega /v1 automáticamente)
